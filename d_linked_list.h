...
template <typename K, typename V> class d_linked_list {
...
template <typename K, typename V> d_linked_list<K, V>::~d_linked_list() {
...
template <typename K, typename V>
typename d_linked_list<K, V>::T const &
d_linked_list<K, V>::get(std::size_t pos) const {
  auto p = head_.next;
  while (pos--)
    p = p->next;
  return p->value;
