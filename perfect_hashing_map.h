...
class perfect_hashing_map
{
	struct uni_hash
	{
		...
		explicit uni_hash(Hash *hash) : hash(hash)
		{
			update();
		}

		uni_hash() : hash(nullptr)
		{
			update();
		}
	...
	Hash hash_;
	std::vector<std::pair<std::vector<std::pair<bool, std::pair<Key, T>>>, uni_hash>> data_;
public:
	size_t n_elements = 0;
	perfect_hashing_map(const std::vector<Key>& keys, Hash hash = Hash()) : hash_(hash), data_(keys.size())
	{
	...
	bool insert(const Key& key, const T& value)
	{
		...
		data_[external_table_pos].first[internal_table_pos].first = true; // occupied = true
	...
	void erase(const Key& key)
	{
		...
		auto internal_table_pos = data_[external_table_pos].second(key)
		% data_[external_table_pos].first.size();
		data_[external_table_pos].first[internal_table_pos].first = false; // occupied = false
		--n_elements;
	}
